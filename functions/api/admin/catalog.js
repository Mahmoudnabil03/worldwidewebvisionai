...
import { CATEGORIES } from '../../../public/catalog.js';
...
function readProduct(raw) {
  ...
  if (!CAT_IDS.includes(cat)) {
    throw new ApiError(400, 'bad_cat', `Category must be one of: ${CAT_IDS.join(', ')}.`, { field: 'cat' });
  }
...
export const onRequestGet = handle(async (context) => {
  ...
  return json({
    ok: true,
    categories: CATEGORIES.map((c) => ({ id: c.id, ar: c.ar, en: c.en })),
    products: results || [],
...
export const onRequestPost = handle(async (context) => {
  ...
  const p = readProduct(body.product);
