...
export const onRequestGet = handle(async (context) => {
  ...
  if (entity === 'orders') {
    const { results } = q
      ? await d1.prepare(
          `SELECT id, created_at, name, phone, governorate, total, status, notified
             FROM orders
    ...
    return json({ ok: true, entity, limit, orders: results || [] });
  }
...
export const onRequestPost = handle(async (context) => {
  ...
  if (entity === 'order') {
    ...
    const row = await d1.prepare('SELECT id, status, user_id FROM orders WHERE id = ?1').bind(id).first();
    ...
    throw new ApiError(400, 'bad_action', 'action must be status, cancel or delete.', { field: 'action' });
  }
